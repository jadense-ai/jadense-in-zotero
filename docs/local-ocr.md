...
**v0.6.5 Windows x64 完整离线包：**[正式 Release](https://github.com/jadense-ai/jadense-in-zotero/releases/tag/v0.6.5) 的 ZIP 包含匹配的插件 XPI 与本机 OCR 引擎 ZIP。按[完整 ZIP 离线安装指南](usage-guide.md#windows-x64-zip-离线安装)安装 XPI 并重启后，在 **设置 → 外置依赖配置 → 本机 OCR → 导入离线包** 选择 `jadense-ocr-engine-*.zip`；等待离线模型验证完成并显示已就绪。无需预装 Python/uv、在线下载依赖或管理员权限。此包包含全文 OCR 模型，选文公式增强仍需首次下载 CodeFormulaV2。
