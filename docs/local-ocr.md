...
> 版本范围：统一准备入口已包含在正式版 **0.4.9**。细分下载进度、超时提示及「删除依赖 / 重新安装 OCR」是 **0.4.10 候选版的改进（待发布）**，v0.4.9 已发布 XPI 不一定有这些控件。源码版本号相同不代表安装包内容相同；旧版以实际界面为准。
...
| 依赖已删除 · 需要重新安装（0.4.10 待发布） | 点击「重新安装 OCR」；历史成果仍可阅读 |
...
3. **修复仍失败**（以下为 0.4.10 待发布按钮）：展开「环境与故障排查 → 删除与重装」，点击「删除依赖…」。默认不勾选「同时删除已下载模型」；怀疑模型损坏或确需释放空间时才勾选。确认后先停止服务，再删除插件专用依赖。
