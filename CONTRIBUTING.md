...
5. 按下方文案规范完善草稿，补齐实际 Zotero/操作系统版本、功能限制和升级注意事项，确认附件齐全。Manifest 兼容范围不能代替实测记录。所有验收项完成后在 GitHub 人工公开草稿。
...
4. Release 标题仅用 `vX.X.X`。正文使用「本次更新 / 操作与配置 / 升级与兼容 / 验证与下载」，描述相对上一正式版的实际变化，链接相应用户指南。
...
建议结构为「本次更新 / 操作与配置 / 升级与兼容 / 验证与下载」。小版本可合并段落，不必机械套用标题。GitHub 自动生成的提交摘要只能作为补充，不能代替面向用户的说明。
