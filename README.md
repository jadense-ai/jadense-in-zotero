...
**未发布源码（0.5.0）：**新增独立文献分类窗口、对话文件附件与长 PDF 分层阅读，全文任务默认使用文字层，OCR 增强按需开启；改进侧栏恢复、诊断与参考文献年份识别。无精确匹配时标记「已选首条」，导入前请核对；首次全文翻译提示费用与稳定性。此源码更新不提供新的正式安装包。入口、配置与限制见[未发布功能指南](docs/usage-guide.md#unreleased-050)。以下安装说明对应正式版；0.5.0 的全文文字层任务无需安装 OCR。
...
从 GitHub 0.4.0–0.4.9 可直接安装 0.4.10；相同 `.cn` 插件身份保留已有设置与本地历史。旧译文不会自动重译。
