...
- Main navigation has `对话 / 翻译历史 / 文献解析 / 连接攻玉 / 上手指南`; Settings remains a section with its gear-icon entry in the sidebar footer. Preserve the internal compatibility section value `migrate` for `连接攻玉`, including old upload/context-menu entry URLs.
...
- `文献解析` has two top bookmark tabs: `解析历史 / 解析配置`. A paper opens an internal detail page with `解析总结 / 解析笔记 / 参考文献`, retaining the Manager sidebar. Shared `ui/controls.ts`, `ui/select.ts` and `content/ui.css` own buttons, notices, badges, custom selects and tabs; the old `custom-select.ts` export remains compatible. Use `tablist`, `tab`, `tabpanel`, one tab stop, arrow keys and Home/End. The active bookmark joins the white reading surface with a restrained green top line; inactive bookmarks use the recessed surface.
...
Manager Settings uses accessible `常规 / 功能配置 / 快捷键设置 / BYOK` tabs in that order, opening `常规` by default. General owns `显示语言` (`跟随 Zotero / 简体中文 / English`), `主题设置` (`跟随 Zotero / 浅色 / 深色`), interface font size, translation window style, and background transparency. Keep all controls in the same settings card, aligned as labeled rows with help text; use a grouped font stepper/reset, the existing style select, and a 0–100% transparency slider with its current value. Language changes save for the next Zotero restart, with an explicit restart hint; theme changes apply immediately across open plugin surfaces. The feature tab lists `AI 对话 / 实时翻译 / 文献解析 / 图片解读`, each with an independent searchable Jadense/BYOK selector. Shortcut recording/save/disable/reset and BYOK editing remain in their own tabs. Account, token, folder, PDF, and upload controls belong to `连接攻玉`.

The composer always exposes both model sources. It edits the Chat selection for text and uploaded images, and the image-interpretation selection when the latest image came from the Reader, so subsequent image questions remain consistent. `文献解析 → 解析配置` edits the same analysis choice as feature settings. Empty/offline model catalogs cannot prevent selecting configured BYOK models. Keep saved unavailable choices visible, with a local actionable error for that feature. Chinese surfaces use `提供商 / 模型 ID / API 基础地址 / API 密钥 / 最大输出量（词元）`; preserve actual protocol and model proper names.
...
- The five main navigation entries remain visible and correctly route to `对话 / 翻译历史 / 文献解析 / 连接攻玉 / 上手指南`; the analysis page exposes accessible `解析历史 / 解析配置 / 参考文献` bookmark tabs.
...
- Settings contains `常规 / 功能配置 / 快捷键设置 / BYOK`, with General first/default, four independent model selectors, editable screenshot/translation chords, and a gear entry. Native Preferences starts with General, then feature, connection, and BYOK sections.
...
Render only complete translated paragraphs. Adjacent unfinished paragraphs form one light status notice; failures preserve a visible gap in both reading and Copy. Put Copy all, history and explicit retranslation in `⋯`; progress/pause remain in the footer and task actions disappear when finished. Closing the view does not pause DocumentJobs. Manager full-document history uses the same reader. Keep raw extraction and old results readable; no request is triggered simply by opening an old record.
