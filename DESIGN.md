...
Manager Settings uses accessible `功能配置 / 快捷键设置 / BYOK` tabs in that order. The first tab lists `AI 对话 / 实时翻译 / 文献解析 / 图片解读`, each with a searchable selector offering Jadense and saved BYOK models; choices save immediately and remain independent. There is no global AI-channel control. The shortcut tab retains capture/translation recording, save, disable, and reset. The BYOK tab manages providers and model definitions. Jadense account, token, folder, PDF, and upload controls belong to `连接攻玉`.
...
Native Zotero Preferences remains the fallback configuration surface when Manager cannot open. It keeps the four feature selectors followed by `连接攻玉` and `BYOK 配置` sections in a single column, backed by the same local preferences; the account, points, check-in, and upload workbench stays Manager-only.
...
- Settings contains `功能配置 / 快捷键设置 / BYOK`, four independent model selectors, editable screenshot/translation chords, and a gear entry; native Preferences retains feature, connection, and BYOK sections.
