...
- `文献解析` is a first-class Manager section with three top bookmark tabs: `解析历史 / 解析配置 / 参考文献`. Use `tablist`, `tab`, and `tabpanel` semantics with keyboard focus and selected state. Inactive tabs use the recessed surface and muted text; the active tab uses the panel surface, rounded top corners only, no outline border, and overlaps the panel edge by about 1px so tab and panel read as one shape.
